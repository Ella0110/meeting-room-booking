...
- 空闲格子：浅色虚线边框（与房间主色对应），可点击 → 滑出右侧预订面板（房间和时间已预填）
- 自己的预订：显示会议主题 + 房间专属颜色填充
- 他人的预订：灰色色块，不显示标题（隐私保护）
- 封锁时段：灰色，不可点击
- 左右箭头切换日期；右上角日/周视图切换
...
**移动端适配（≤ 768px）**
- 主页切换为纵向布局：顶部房间 Tab 切换，下方显示该房间当天时间轴
- 时间轴纵向滚动，时段块可点击 → 底部抽屉（bottom sheet）弹出预订表单
- 导航收起为底部 Tab Bar（日历 / 我的预订 / 头像）
- 管理后台移动端仅展示基础只读视图，操作建议在桌面端进行

**高并发策略**
- 前端：React Query 全局缓存 + 乐观更新（预订创建后立即更新本地状态，失败则回滚）
- 前端：同一时间段内去重请求，避免用户快速点击触发多次提交（按钮禁用 + 请求锁）
- 后端：数据库事务 + `SELECT ... FOR UPDATE` 行级锁，保证并发安全
- 后端：Prisma 连接池，合理配置最大连接数
- 后端：创建预订接口加接口级限流（同一用户 5 秒内最多 3 次请求）
